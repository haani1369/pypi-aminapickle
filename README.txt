...
the motivation behind it roughly comes from a lecture about supply
chain security we had in MIT's 6.5660 (Secure Computer Systems), given
by Russ Cox. we touched on some of the problems NPM has had lately, and
how package managers for languages such as Python, Go, and Rust try to
fight supply chain attacks

as far as all that goes, this repo is just a cute example of tooling
that can help devs

the name comes from pypi (python package index), and the question
"am i in a pickle?"...
